...
public class Main {
    ...
    public Main() throws IOException {
        ...
        String scoreFilename = "score.txt";//console.readLine();
        ...
        String filename1 = "seq1.fasta";//console.readLine();

        ArrayList<String> sequences1 = new ArrayList<>();
        ArrayList<String> sequences2 = new ArrayList<>();
        readSequenceFromFile(filename1, sequences1);


        String filename2 = "seq2.fasta";// console.readLine();
        ...
        try {
            int i=0;
            while(i<sequences1.size()){
                long start_time = System.nanoTime();
                if ( nmbs.length == 1 ) {
                    table = linear_table(MAXIMIZING, sequences1.get(i), sequences2.get(i), score, gapCost);
                    alignments = getAlignment(sequences1.get(i), sequences2.get(i), table);

                } else {
                    table = affine_table(MAXIMIZING, sequences1.get(i), sequences2.get(i), score, gapCost);
                    alignments = getAffineAlignment(sequences1.get(i), sequences2.get(i), table);
                }
           ...
           // System.out.println("The alignment was written to solution.fastalignment");
            double[] elpsTime = new double[elapsedTime.size()];
            double[] nM = new double[nPlusM.size()];

            for (int j = 0; j < elpsTime.length; j++) {
                elpsTime[j] = elapsedTime.get(j);                // java 1.5+ style (outboxing)
                nM[j] = nPlusM.get(j);                // java 1.5+ style (outboxing)
            }
            SimpleRealTime simpleRealTime1 = new SimpleRealTime(nM, elpsTime, typeOfGapCost);
            simpleRealTime1.makeGraph();
...
//    }
    public static void readSequenceFromFile(String filename, ArrayList<String> sequence) throws FileNotFoundException {
        ...
        sc.useDelimiter("\n");
        while (sc.hasNext()) {
            sequence.addAll(Arrays.asList(sc.next().trim()));
        }
    }
